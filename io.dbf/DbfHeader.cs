...
namespace io.dbf
{
    public class DbfHeader : ICloneable
    {
        ...
        private const int FileType = 0x03;
        ...
        //DBF fields/columns
        internal List<DbfColumn> Fields = new List<DbfColumn>();
        ...
        private Dictionary<string, int> _columnNameIndex;
        ...
        /// <inheritdoc />
        public DbfHeader(Encoding encoding) => Encoding = encoding;
        ...
        {
            Fields = new List<DbfColumn>(nFieldCapacity);
        ...
        /// Gets header length.
        /// </summary>
        public ushort HeaderLength => _headerLength;
        ...
        public void RemoveColumn(int nIndex)
        {
            ...
            DbfColumn oColRemove = Fields[nIndex];
            ...
            for (int i = nIndex; i < Fields.Count; i++)
                Fields[i]._dataAddress -= nRemovedColLen;
        ...
        /// <returns></returns>
        public DbfColumn this[int nIndex] => Fields[nIndex];
        ...
        public int FindColumn(string sName)
        {
            ...
            {
                _columnNameIndex = new Dictionary<string, int>(Fields.Count);

                //create a new index
                for (int i = 0; i < Fields.Count; i++)
                {
                    _columnNameIndex.Add(Fields[i].Name.ToUpper(), i);
                }
            }

            if (_columnNameIndex.TryGetValue(sName.ToUpper(), out var columnIndex))
                return columnIndex;
        ...
        /// 
        /// </remarks>
        protected internal byte[] EmptyDataRecord => _emptyRecord ?? (_emptyRecord = Encoding.GetBytes("".PadLeft(_recordLength, ' ').ToCharArray()));


        /// <summary>
        /// Returns Number of columns in this dbf header.
        /// </summary>
        public int ColumnCount => Fields.Count;
        ...
        public void Write(BinaryWriter writer)
        {
            ...
            foreach (DbfColumn field in Fields)
            {
        ...
        public void Read(BinaryReader reader)
        {
            ...
            Fields = new List<DbfColumn>(nNumFields);
            for (int i = 0; i < nNumFields; i++)
            {
                ...
                Fields.Add(new DbfColumn(sFieldName, DbfColumn.GetDbaseType(cDbaseType), nFieldLength, nDecimals, nDataOffset));
            ...
            int nExtraReadBytes = _headerLength - (FileDescriptorSize + (ColumnDescriptorSize * Fields.Count));
